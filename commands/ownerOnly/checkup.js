...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('checkup')
        .setDescription('The Wabbit analyzes the server and gives You an score between 1 to 10'),
    async execute(interaction) {
        ...
        if (interaction.user.id === owner.id || dev.includes(interaction.user.id)) {
            ...
            let totalsco = 10
            let embedse = []
            let WabbitAppId = '724910037486534696'
            let WabbitUserId = '904822152245694555'
            ...
            interaction.guild.members.cache.filter(member => member.user.bot).forEach(m => {
                if (m.user.id == WabbitUserId) return;
            ...
            if (totalsco < 10) {
                interaction.reply({ content: `\`📩 Your Score Is ${totalsco}/10\``, embeds: embedse, ephemeral: true })
            } else if (totalsco == 10) {
                interaction.reply({ content: `\`📩 Your Score Is ${totalsco}/10\` And No Problem was Found`, embeds: embedse, ephemeral: true })
            }
