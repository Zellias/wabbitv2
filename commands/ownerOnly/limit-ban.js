...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addSubcommand(subcommand =>
            subcommand
                .setName('add')
                .setDescription('Set Limits of Ban Members')
    ...
    async execute(interaction) {
        ...
        switch (sub) {
            case 'add':
                ...
                resualt.limit.ban.add = channeCreate
                await resualt.save()

                embed.setDescription(`
                <:983651050911313941:983651050911313941> | ban Limit has sucsessfully set to ${channeCreate}
            `)
            ...
            default:
                ...
                console.log(value)
                if (value === 'on') {
                    resualt.limit.ban.isEnable = true
                    await resualt.save()
                    embed.setDescription(`
                    <:983651050911313941:983651050911313941> | Anti Ban Status has successfully set to ${value}
                        `)
                    await interaction.reply({ content: `${interaction.user}`, embeds: [embed], ephemeral: true });
                } else if (value === 'off') {
                    resualt.limit.ban.isEnable = false
                    await resualt.save()
                    embed.setDescription(`
                    <:983651050911313941:983651050911313941> | Anti Ban Status has successfully set to ${value}
                        `)
