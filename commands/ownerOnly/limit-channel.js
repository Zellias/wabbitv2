...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .setDescription('Set Limits of channel [ Create / Delete ]')

        .addSubcommand(subcommand =>
            subcommand
                .setName('create')
                .setDescription('Set Limits of Channel creations')
                .addIntegerOption(option => option.setName('limit').setDescription('Enter Limit Number').setRequired(true)))
        .addSubcommand(subcommand =>
            subcommand
                .setName('delete')
                .setDescription('Set Limits of Channel Delete')
                .addIntegerOption(option => option.setName('limit').setDescription('Enter Limit Number').setRequired(true)))
        .addSubcommand(subcommand =>
            subcommand
                .setName('status')
    ...
    async execute(interaction) {
        ...
        switch (sub) {
            case 'create':
                ...
                resualt.limit.channel.create = channeCreate
                await resualt.save()

                embed.setDescription(`
            Channel Create Limit has sucsessfully set to ${channeCreate}
            `)
            ...
            case 'delete':
                ...
                resualt.limit.channel.delete = channeDelete
                await resualt.save()
                embed.setDescription(`
                Channel Delete Limit has sucsessfully set to ${channeDelete}
                `)
                await interaction.reply({ content: `${interaction.user}`, embeds: [embed], ephemeral: true });
            ...
            default:
                ...
                console.log(value)
                if (value === 'on') {
                    resualt.limit.channel.isEnable = true
                    await resualt.save()
                    embed.setDescription(`
                    <:983651050911313941:983651050911313941> | Anti channel Status has successfully set to ${value}
                    `)
                    await interaction.reply({ content: `${interaction.user}`, embeds: [embed], ephemeral: true });
                } else if (value === 'off') {
                    resualt.limit.channel.isEnable = false
                    await resualt.save()
                    embed.setDescription(`
                    <:983651050911313941:983651050911313941> | Anti channel Status has successfully set to ${value}
                    `)
